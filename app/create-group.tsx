...
export default function CreateGroupScreen() {
  const { people, createGroup, isBlocked } = useApp();
  ...
  const filtered = useMemo(() => people.filter(person => !isBlocked(person.id) && `${person.displayName} ${person.username} ${person.interests.join(' ')}`.toLowerCase().includes(query.toLowerCase())), [people, query, isBlocked]);
  ...
  return (
    <SafeAreaView style={styles.screen}>
      <View style={styles.progress}><View style={styles.progressTrack}><View style={[styles.progressFill, { width: step === 1 ? '50%' : '100%' }]} /></View><Text style={styles.progressText}>{t('group.step', { step })}</Text></View>
      {step === 1 ? <>
        <View style={styles.heading}><Text style={styles.title}>{t('group.whosIn')}</Text><Text style={styles.subtitle}>{t('group.choose')}</Text></View>
        <View style={styles.search}><SearchField placeholder={t('group.search')} value={query} onChangeText={setQuery} /></View>
        {selected.length > 0 && <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.selected}>{selected.map(id => { const user = people.find(person => person.id === id)!; return <Pressable accessibilityRole="button" accessibilityLabel={t('group.remove', { name: user.displayName })} key={id} style={styles.selectedPerson} onPress={() => toggle(id)}><Avatar label={user.avatar} size={44} /><View style={styles.remove}><Ionicons name="close" size={11} color={colors.inverseText} /></View><Text numberOfLines={1} style={styles.selectedName}>{user.displayName.split(' ')[0]}</Text></Pressable>; })}</ScrollView>}
        <ScrollView contentContainerStyle={styles.people}>{filtered.map(person => {
          const active = selected.includes(person.id);
          return <Pressable accessibilityRole="checkbox" accessibilityLabel={t('group.select', { name: person.displayName })} accessibilityState={{ checked: active }} key={person.id} onPress={() => toggle(person.id)} style={styles.person}><Avatar label={person.avatar} size={50} /><View style={styles.personMain}><Text style={styles.personName}>{person.displayName}</Text><Text style={styles.personMeta}>@{person.username} · {person.interests[0]}</Text></View><View style={[styles.check, active && styles.checkActive]}>{active && <Ionicons name="checkmark" size={17} color={colors.inverseText} />}</View></Pressable>;
        })}</ScrollView>
        <View style={styles.footer}><PrimaryButton title={selected.length ? t('group.continue', { count: selected.length }) : t('group.selectPeople')} onPress={() => setStep(2)} disabled={selected.length === 0} /></View>
      </> : <>
        <ScrollView contentContainerStyle={styles.details} keyboardShouldPersistTaps="handled">
          <Pressable accessibilityRole="button" accessibilityLabel={t('group.changeLabel')} style={styles.backStep} onPress={() => setStep(1)}><Ionicons name="arrow-back" size={18} color={colors.text} /><Text style={styles.backText}>{t('group.change')}</Text></Pressable>
          <View><Text style={styles.title}>{t('group.nameTitle')}</Text><Text style={styles.subtitle}>{t('group.nameHelp')}</Text></View>
          <View style={styles.groupAvatar}><View style={styles.groupCircle}><Text style={styles.groupInitials}>{name.trim().slice(0, 2).toUpperCase() || 'PG'}</Text></View></View>
          <Field label={t('group.name')} placeholder={t('group.namePlaceholder')} value={name} onChangeText={setName} maxLength={48} autoFocus />
          <View style={styles.membersCard}><View style={styles.membersHead}><Text style={styles.membersTitle}>{t('group.members')}</Text><Text style={styles.membersCount}>{selected.length + 1}</Text></View><View style={styles.ownerRow}><Avatar label="A" size={36} /><Text style={styles.ownerName}>{t('common.you')}</Text><Text style={styles.role}>{t('common.owner')}</Text></View>{selected.map(id => { const user = people.find(person => person.id === id)!; return <View key={id} style={styles.ownerRow}><Avatar label={user.avatar} size={36} /><Text style={styles.ownerName}>{user.displayName}</Text><Text style={styles.role}>{t('common.member')}</Text></View>; })}</View>
        </ScrollView>
        <View style={styles.footer}><PrimaryButton title={t('group.create')} onPress={finish} disabled={!name.trim()} /></View>
      </>}
    </SafeAreaView>
  );
}
...
const createStyles = (colors: ThemeColors) => StyleSheet.create({
  ...
  groupAvatar: { alignItems: 'center', gap: 9 }, groupCircle: { width: 88, height: 88, borderRadius: 28, backgroundColor: colors.soft, borderWidth: 1, borderColor: colors.line, alignItems: 'center', justifyContent: 'center' }, groupInitials: { color: colors.text, fontSize: 26, fontWeight: '800' },
