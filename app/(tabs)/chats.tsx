...
export default function ChatsScreen() {
  ...
  return (
    <SafeAreaView style={styles.screen}>
      <View style={styles.header}><View><Text style={styles.kicker}>YOUR PEOPLE</Text><Text style={styles.title}>Chats</Text></View><IconButton name="add" onPress={() => setMenu(true)} style={styles.add} /></View>
      ...
      <FlatList data={filtered} keyExtractor={item => item.id} contentContainerStyle={styles.list} ItemSeparatorComponent={() => <View style={styles.separator} />}
        ...
        renderItem={({ item }) => (
          <Pressable style={({ pressed }) => [styles.row, pressed && { opacity: .65 }]} onPress={() => router.push({ pathname: '/chat/[id]', params: { id: item.id } })}>
      ...
      <Modal visible={menu} transparent animationType="slide" onRequestClose={() => setMenu(false)}>
        <Pressable style={styles.overlay} onPress={() => setMenu(false)}><Pressable style={styles.sheet}>
          <View style={styles.handle} /><Text style={styles.sheetTitle}>Start something</Text>
          <SheetItem icon="person-add-outline" title="New Chat" caption="Find someone on Milo" onPress={() => { setMenu(false); router.push('/(tabs)/discover'); }} />
...
function SheetItem({ icon, title, caption, onPress }: { icon: keyof typeof Ionicons.glyphMap; title: string; caption: string; onPress: () => void }) {
  return <Pressable onPress={onPress} style={styles.sheetRow}><View style={styles.sheetIcon}><Ionicons name={icon} size={21} color={colors.accent} /></View><View><Text style={styles.sheetItemTitle}>{title}</Text><Text style={styles.sheetCaption}>{caption}</Text></View></Pressable>;
}
...
const styles = StyleSheet.create({
  ...
  kicker: { color: colors.accent, fontSize: 11, fontWeight: '700', letterSpacing: 1.2, marginBottom: 4 }, title: { fontSize: 34, fontWeight: '800', letterSpacing: -1.2, color: colors.text },
  add: { backgroundColor: colors.accentSoft }, search: { paddingHorizontal: spacing.lg, paddingBottom: spacing.md }, list: { paddingHorizontal: spacing.lg, paddingBottom: 30 },
  ...
  badge: { minWidth: 21, height: 21, borderRadius: 11, backgroundColor: colors.accent, alignItems: 'center', justifyContent: 'center', paddingHorizontal: 6 }, badgeText: { color: 'white', fontWeight: '700', fontSize: 11 }, separator: { height: 1, backgroundColor: colors.line, marginLeft: 68 },
  ...
  overlay: { flex: 1, justifyContent: 'flex-end', backgroundColor: '#00000035' }, sheet: { backgroundColor: colors.surface, borderTopLeftRadius: 24, borderTopRightRadius: 24, padding: 24, paddingBottom: 38 },
  ...
  sheetRow: { flexDirection: 'row', alignItems: 'center', gap: 14, paddingVertical: 13 }, sheetIcon: { width: 44, height: 44, borderRadius: 14, backgroundColor: colors.accentSoft, alignItems: 'center', justifyContent: 'center' },
  sheetItemTitle: { color: colors.text, fontWeight: '600', fontSize: 16 }, sheetCaption: { color: colors.secondary, fontSize: 13, marginTop: 3 },
  toast: { position: 'absolute', bottom: 28, alignSelf: 'center', backgroundColor: colors.text, paddingHorizontal: 18, paddingVertical: 11, borderRadius: radius.pill }, toastText: { color: 'white', fontSize: 13, fontWeight: '600' },
});
