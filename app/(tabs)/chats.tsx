...
export default function ChatsScreen() {
  ...
  const [toast, setToast] = useState(created === '1' ? t('chats.created') : '');
  ...
  return (
    <SafeAreaView style={styles.screen}>
      <View style={styles.header}><View><Text style={styles.kicker}>{t('chats.kicker')}</Text><Text style={styles.title}>{t('tabs.chats')}</Text></View><IconButton name="add" accessibilityLabel={t('chats.newMenu')} onPress={() => setMenu(true)} style={styles.add} /></View>
      <View style={styles.search}><SearchField placeholder={t('chats.search')} value={query} onChangeText={setQuery} /></View>
      <FlatList data={filtered} keyExtractor={item => item.id} contentContainerStyle={styles.list} ItemSeparatorComponent={() => <View style={styles.separator} />}
        ListEmptyComponent={<View style={styles.empty}><Ionicons name="chatbubbles-outline" size={32} color={colors.muted} /><Text style={styles.emptyTitle}>{t('chats.emptyTitle')}</Text><Text style={styles.emptyText}>{t('chats.emptyText')}</Text></View>}
        renderItem={({ item }) => (
          <Pressable accessibilityRole="button" accessibilityLabel={t('chats.open', { name: item.title })} style={({ pressed }) => [styles.row, pressed && { opacity: .65 }]} onPress={() => router.push({ pathname: '/chat/[id]', params: { id: item.id } })}>
            <Avatar label={item.avatar} size={54} />
            <View style={styles.rowMain}><View style={styles.rowTop}><Text style={styles.name}>{item.title}</Text><Text style={styles.time}>{formatConversationTime(item.updatedAt, Date.now(), locale, t('date.yesterday'))}</Text></View><View style={styles.rowBottom}><Text numberOfLines={1} style={styles.message}>{item.lastMessage}</Text>{item.unreadCount > 0 && <View style={styles.badge}><Text style={styles.badgeText}>{item.unreadCount}</Text></View>}</View></View>
          </Pressable>
        )} />
      <Modal visible={menu} transparent animationType="slide" onRequestClose={() => setMenu(false)}>
        <Pressable accessibilityRole="button" accessibilityLabel={t('chat.closeActions')} style={styles.overlay} onPress={() => setMenu(false)}><Pressable style={styles.sheet}>
          <View style={styles.handle} /><Text style={styles.sheetTitle}>{t('chats.newMenu')}</Text>
          <SheetItem icon="person-add-outline" title={t('chats.newChat')} caption={t('chats.newChatCaption')} onPress={() => { setMenu(false); router.push('/(tabs)/discover'); }} />
          <SheetItem icon="people-outline" title={t('chats.newGroup')} caption={t('chats.newGroupCaption')} onPress={() => { setMenu(false); router.push('/create-group'); }} />
        </Pressable></Pressable>
      </Modal>
