  ...
  return (
    <KeyboardAvoidingView style={styles.screen} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
        <View><Text style={styles.step}>YOUR MILO IDENTITY</Text><Text style={styles.title}>Let people know who you are.</Text><Text style={styles.subtitle}>You can change these details any time.</Text></View>
        <View style={styles.avatarWrap}>
          <Avatar label={name || 'M'} size={92} />
          <View style={styles.camera}><Ionicons name="camera-outline" size={17} color="white" /></View>
...
const styles = StyleSheet.create({
  ...
  step: { color: colors.accent, fontWeight: '700', letterSpacing: 1.2, fontSize: 12, marginBottom: 12 },
  ...
  avatarWrap: { alignItems: 'center', gap: 10 }, camera: { width: 30, height: 30, borderRadius: 15, backgroundColor: colors.accent, alignItems: 'center', justifyContent: 'center', marginTop: -33, marginLeft: 65, borderWidth: 3, borderColor: colors.background },
  optional: { color: colors.secondary, fontSize: 13 }, form: { gap: 20 }, error: { color: colors.danger, fontSize: 13 },
